...
<tr wire:key="{{ $item->id . $page }}">
    ...
    <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900 dark:text-white">
        ...
        <div class="inline-flex rounded-lg border border-gray-50 bg-gray-100 dark:border-gray-800 dark:bg-gray-900">
            @if(($routeView))
            ...
            <button wire:click="$dispatch('openModal', { component: {{$componentEditName}} , arguments: {id: {{$item->id}} }})" class="inline-flex items-center gap-1 rounded-md px-2 text-sm text-gray-500 bg-gray-700 hover:bg-gray-400 focus:relative dark:text-gray-50 dark:hover:text-gray-100 m-1">
                <x-heroicon-s-pencil class="w-6 h-8 p-1 text-yellow-500" />
                edit
            </button>
            @else
            ...
            <button wire:loading.attr="disabled" wire:click="$dispatch('delete-confirmation', {id: {{ $item->id }}})" class="inline-flex items-center gap-1 rounded-md px-2 text-sm text-gray-500 bg-gray-700 hover:bg-gray-400 focus:relative dark:text-gray-50 dark:hover:text-gray-100 m-1">
                <x-heroicon-s-trash class="w-6 h-8 p-1 text-red-500" />
                hapus
            </button>
        </div>
